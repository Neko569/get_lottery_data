...
"""
获取福彩快乐八开奖数据
数据源: https://kaijiang.500.com/static/info/kaijiang/xml/kl8/list.xml

快乐八：每期开20个号码（01-80中选20个），每日开奖

用法:
    python kl8_fetcher.py --latest    # 获取最新一期
    python kl8_fetcher.py --history   # 获取全部历史数据
"""
...
API_URL = "https://kaijiang.500.com/static/info/kaijiang/xml/kl8/list.xml"
...
def parse_record(row):
    """解析单条 XML row，快乐八每期20个号码

    XML 格式：<row expect="2026166" opencode="02,08,14,..." opentime="2026-06-25 21:30:00"/>
    """
...
def update_latest():
    filepath = os.path.join(DATA_DIR, "kl8_history.json")
    latest = get_latest()
    if not latest:
        print("未能获取到最新数据")
        return False
...
def save_to_file(records, filename="kl8_history.json"):
...
def save_to_csv(records, filename="kl8_history.csv"):
    if not records:
        return
    filepath = os.path.join(DATA_DIR, filename)
...
def main():
    parser = argparse.ArgumentParser(description="获取快乐八开奖数据")
    ...
    if args.latest:
        ...
        print("获取快乐八最新一期并增量更新")
        update_latest()
    else:
        months = args.recent if args.recent else None
        if months:
            print(f"获取快乐八最近 {months} 个月数据")
        else:
            print("获取快乐八全部历史数据")
        print("=" * 50)
        ...
        if records:
            if args.dry_run:
                ...
                save_to_file(records, "kl8_history.json")
                save_to_csv(records, "kl8_history.csv")
            print(f"\n共 {len(records)} 条记录")
        else:
            print("未能获取到数据")
    print("=" * 50)
