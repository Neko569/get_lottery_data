...
"""
获取福彩3D开奖数据
数据源: https://kaijiang.500.com/static/info/kaijiang/xml/sd/list.xml

福彩3D：3个数字（0-9），每日开奖

用法:
    python fc3d_fetcher.py --latest    # 获取最新一期
    python fc3d_fetcher.py --history   # 获取全部历史数据
"""
...
API_URL = "https://kaijiang.500.com/static/info/kaijiang/xml/sd/list.xml"
...
def parse_record(row):
    """解析单条 XML row，福彩3D 3个数字（0-9）

    XML 格式：<row expect="2026166" opencode="9,0,0" opentime="2026-06-25 21:15:00"/>
    """
...
def save_to_file(records, filename="fc3d_history.json"):
...
def save_to_csv(records, filename="fc3d_history.csv"):
    if not records:
        return
    filepath = os.path.join(DATA_DIR, filename)
...
def main():
    parser = argparse.ArgumentParser(description="获取福彩3D开奖数据")
    ...
    if args.latest:
        ...
        print("获取福彩3D最新一期并增量更新")
        update_latest()
    else:
        months = args.recent if args.recent else None
        if months:
            print(f"获取福彩3D最近 {months} 个月数据")
        else:
            print("获取福彩3D全部历史数据")
        print("=" * 50)
        ...
        if records:
            if args.dry_run:
                ...
                save_to_file(records, "fc3d_history.json")
                save_to_csv(records, "fc3d_history.csv")
            print(f"\n共 {len(records)} 条记录")
        else:
            print("未能获取到数据")
    print("=" * 50)
