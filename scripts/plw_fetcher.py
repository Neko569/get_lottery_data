...
"""
获取体彩排列五开奖数据
数据源: https://kaijiang.500.com/static/info/kaijiang/xml/plw/list.xml

排列五：5个数字（0-9），每日开奖

用法:
    python plw_fetcher.py --latest    # 获取最新一期
    python plw_fetcher.py --history   # 获取全部历史数据
"""
...
API_URL = "https://kaijiang.500.com/static/info/kaijiang/xml/plw/list.xml"
...
def parse_record(row):
    """解析单条 XML row，排列五5个数字（0-9）

    XML 格式：<row expect="26166" opencode="2,2,9,5,0" opentime="2026-06-25 21:25:00"/>
    """
...
def update_latest():
    filepath = os.path.join(DATA_DIR, "plw_history.json")
    latest = get_latest()
    if not latest:
        print("未能获取到最新数据")
        return False
...
def save_to_file(records, filename="plw_history.json"):
...
def save_to_csv(records, filename="plw_history.csv"):
    if not records:
        return
    filepath = os.path.join(DATA_DIR, filename)
...
def main():
    parser = argparse.ArgumentParser(description="获取排列五开奖数据")
    ...
    if args.latest:
        ...
        print("获取排列五最新一期并增量更新")
        update_latest()
    else:
        months = args.recent if args.recent else None
        if months:
            print(f"获取排列五最近 {months} 个月数据")
        else:
            print("获取排列五全部历史数据")
        print("=" * 50)
        ...
        if records:
            if args.dry_run:
                ...
                save_to_file(records, "plw_history.json")
                save_to_csv(records, "plw_history.csv")
            print(f"\n共 {len(records)} 条记录")
        else:
            print("未能获取到数据")
    print("=" * 50)
